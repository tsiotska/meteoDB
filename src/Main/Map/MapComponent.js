...
class MapComponent extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        const {markers, setCardItem, stations} = this.props;

        if (markers && markers.length === 1) {
            setCardItem(stations[0].props.props);
            $('.leaflet-marker-icon').addClass('coloredSelectedMarker')
        }
    }
    ...
    renderMarkers = () => {
        ...
        if (markers.length > 0) {
            return markers.map((w) => this.renderOne(w));
        } else {
            return null;
        }
    };
